...
namespace TrailModUpdated
{
    public class ItemTrowel : Item
    {
        ...
        readonly List<BlockPos> blockHighlightPositions = new List<BlockPos>();
        readonly List<int> blockHighlightColors = new List<int>();
        ...
        public override void OnLoaded(ICoreAPI api)
        {
            ...
            interactions = ObjectCacheUtil.GetOrCreate(api, "trowelInteractions", () =>
            {
                ...
                {
                    new WorldInteraction() { ActionLangCode = "trailmodupdated:heldhelp-trampleprotectblock", MouseButton = EnumMouseButton.Right },
                    new WorldInteraction() { ActionLangCode = "trailmodtrailmodupdated:heldhelp-removetrampleprotection", MouseButton = EnumMouseButton.Left }
                };
            });
        }
        ...
        {
            if (modeItems == null && capi != null)
            {
                modeItems = new[]
                {
            new SkillItem()
            {
                Code = new AssetLocation("single"),
                Name = Lang.Get("Single block")
            },
            new SkillItem()
            {
                Code = new AssetLocation("select"),
                Name = Lang.Get("Protect area")
            },
            new SkillItem()
            {
                Code = new AssetLocation("selectremove"),
                Name = Lang.Get("Unprotect area")
            }
        };

                // Load textures
                modeItems[0].Texture = capi.Gui.LoadSvgWithPadding(
                    new AssetLocation("trailmodupdated:textures/icons/single.svg"), 48, 48, 5, ColorUtil.WhiteArgb);
                modeItems[1].Texture = capi.Gui.LoadSvgWithPadding(
                    new AssetLocation("trailmodupdated:textures/icons/select.svg"), 48, 48, 5, ColorUtil.WhiteArgb);
                modeItems[2].Texture = capi.Gui.LoadSvgWithPadding(
                    new AssetLocation("trailmodupdated:textures/icons/selectremove.svg"), 48, 48, 5, ColorUtil.WhiteArgb);
            }
            return modeItems;
        }
        ...
        public override void OnHeldInteractStart(ItemSlot slot, EntityAgent byEntity, BlockSelection blockSel, EntitySelection entitySel, bool firstEvent, ref EnumHandHandling handling)
        {
            ...
            if (byEntity.World.Side == EnumAppSide.Server)
            {
                ...
                {
                    if (mode == 1) TryProtectArea(byEntity, start, end);
                    else if (mode == 2) TryUnprotectArea(byEntity, start, end);
                }
            }
        ...
        void HandleSingleAdd(EntityAgent byEntity, BlockSelection blockSel, ref EnumHandHandling handling)
        {
        ...
        void HandleSingleRemove(EntityAgent byEntity, BlockSelection blockSel, ref EnumHandHandling handling)
        {
            if (byEntity.World.Side == EnumAppSide.Client)
            {
                handling = EnumHandHandling.PreventDefaultAction;
        ...
        void TryProtectArea(EntityAgent byEntity, BlockPos a, BlockPos b)
        {
            ...
            for (int x = minX; x <= maxX; x++)
                for (int y = minY; y <= maxY; y++)
                    for (int z = minZ; z <= maxZ; z++)
                    {
                        ...
                        if (!block.HasBehavior<BlockBehaviorTrampleProtection>()) continue;
                        if (modTramplePro.IsTrampleProtected(pos)) continue;

                        if (modTramplePro.TryAddTrampleProtection(pos, splayer)) added++;
                    }
            ...
            splayer.SendMessage(GlobalConstants.InfoLogChatGroup, Lang.Get("Protected {0} blocks from trampling.", added), EnumChatType.Notification);
        ...
        void TryUnprotectArea(EntityAgent byEntity, BlockPos a, BlockPos b)
        {
            ...
            for (int x = minX; x <= maxX; x++)
                for (int y = minY; y <= maxY; y++)
                    for (int z = minZ; z <= maxZ; z++)
                    {
                        ...
                        string error = "";
                        if (modTramplePro.TryRemoveTrampleProtection(pos, splayer, ref error)) removed++;
                    }
            ...
            splayer.SendMessage(GlobalConstants.InfoLogChatGroup, Lang.Get("Removed trample protection from {0} blocks.", removed), EnumChatType.Notification);

            ClearSelection(byEntity.World, splayer, true);
        }

        // Helpers
        BlockPos GetSelectionPos(EntityAgent byEntity, string key)
        {
            return byEntity?.WatchedAttributes?.GetBlockPos(key);
        }

        void SetSelectionPos(EntityAgent byEntity, string key, BlockPos pos)
        {
        ...
        void UpdateSelectionOverlayClient(EntityAgent byEntity)
        {
            ...
            bool movedFar = lastOverlayCenter == null
            ...
            int r2 = r * r;
            int minX = center.X - r, maxX = center.X + r;
            int minZ = center.Z - r, maxZ = center.Z + r;
            ...
            for (int x = minX; x <= maxX; x++)
            {
                ...
                for (int z = minZ; z <= maxZ; z++)
                {
                    ...
                    {
                        int dy = y - center.Y;
                        if (dx2 + dz2 + dy * dy > r2) continue;

                        var pos = new BlockPos(x, y, z);
                        if (!modTrample.IsTrampleProtected(pos)) continue;
        ...
        private bool BlockPosEqual(BlockPos a, BlockPos b)
        {
